...
class Zonos(nn.Module):
    ...
    def generate(
        ...
        # Handle Python 3.12+ by conditionally using torch.compile
        if not disable_torch_compile:
            try:
                # Try to use torch.compile, but catch any errors related to Python version
                ...
                if "Dynamo is not supported" in str(e):
                    print("Warning: torch.compile not supported on this Python version. Using uncompiled version.")
                else:
                    raise
